...
function multiplyBy(by:number, ...number:number[]) {
...
// Exactly 3 numbers
function processTriplet(...numbers: [number, number, number]): number {
...
function displayUser({ name, age, email }: { name: string; age: number; email: string }): void {
